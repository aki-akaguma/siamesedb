...
fn open_db_map(db_name: &str) -> Result<FileDbMapDbBytes, std::io::Error> {
    let db = siamesedb::open_file(db_name).unwrap();
    db.db_map_bytes_with_params(
        "some_map1",
        FileDbParams {
...
fn conv_to_kv_string(ki: i64, _vi: i64) -> (DbBytes, String) {
...
fn _test_write(db_name: &str) -> Result<(), std::io::Error> {
    ...
    //
    let mut kv_vec: Vec<(DbBytes, String)> = Vec::new();
    let mut ki: i64 = 0;
    loop {
        ki += 1;
        if ki > LOOP_MAX {
...
fn _test_read(db_name: &str) -> Result<(), std::io::Error> {
    ...
    //
    let mut key_vec: Vec<DbBytes> = Vec::new();
